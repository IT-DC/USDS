...
try
{
	path p(rootPath.c_str());
	if (!exists(p))
		throw usds::ErrorMessage(3, "Directory '") << rootPath.c_str() << "' does not exist";
	if (!is_directory(p))
		throw usds::ErrorMessage(4, "Path '") << rootPath.c_str() << "' is not a directory";
	...
	while (it != endit)
	{
		...
		{
			auto it_ext = find(ext.begin(), ext.end(), it->path().extension());
			if (it_ext != ext.end())
			{
				boost::filesystem::ifstream file(it->path());
				// read file to output list
	...
	if (outList->size() == 0)
		throw usds::ErrorMessage(5, "Code files not found in '") << rootPath.c_str();
